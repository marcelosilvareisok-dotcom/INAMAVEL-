...
export default function PublishModal({ isOpen, onClose, onPublish, projectName, projectSlug }: PublishModalProps) {
  ...
  const shareUrl = `${window.location.origin}/p/${projectSlug}`;
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold text-white">
                {success ? 'Projeto Publicado!' : `Publicar ${projectName}`}
              </h2>
            ...
            {success ? (
              <div className="space-y-4">
                ...
                <p className="text-sm text-white/60 text-center">
                  Seu projeto foi publicado com sucesso no GitHub e no servidor!
                </p>
                <a
                  href={linkedInUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full py-2.5 bg-[#0077B5] text-white text-sm font-medium rounded-lg hover:bg-[#006396] transition-all flex items-center justify-center gap-2"
                >
                  <Linkedin size={16} />
                ...
                <button
                  onClick={onClose}
                  className="w-full py-2.5 bg-white/5 text-white text-sm font-medium rounded-lg hover:bg-white/10 transition-all"
                >
                  Fechar
                </button>
              </div>
